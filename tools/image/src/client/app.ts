...
import { decode, draw, encode, isHeic, type Target, type CropRect } from './ops.js';
import {
  resolveResize,
  scaleByPercent,
  renameForType,
...
type Magic = { $refs: Record<string, HTMLCanvasElement>; $nextTick: (cb: () => void) => void };
  ...
  return {
    ...
    addFiles(list: FileList | File[]) {
      for (const file of Array.from(list)) {
        if (!/^image\//.test(file.type) && !isHeic(file) && !/\.(svg|avif|bmp)$/i.test(file.name))
          continue;
        this.items.push({
          id: nextId++,
          file,
    ...
    async processOne(it: Item) {
      ...
      try {
        ...
        it.status = 'error';
        it.error = e instanceof Error ? e.message : 'Could not process this file';
      }
    },
    ...
    async downloadAll() {
      ...
      for (const it of done) {
        let name = it.outName;
        let n = 2;
        while (entries[name]) name = it.outName.replace(/(\.[^.]+)$/, `-${n++}$1`);
  ...
  return {
    ...
    async load(list: FileList | File[]) {
      ...
      try {
        ...
        this.error = e instanceof Error ? e.message : 'Could not open this image';
        this.file = null;
        this.bmp = null;
      }
    },
    ...
    paint() {
      ...
      const maxW = Math.min(canvas.parentElement!.clientWidth, 720);
      ...
      if (this.sel) {
        ...
        ctx.lineWidth = 2;
        ctx.strokeRect(this.sel.x + 1, this.sel.y + 1, this.sel.w - 2, this.sel.h - 2);
      }
    },
    ...
    async download() {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = renameForType(this.file.name.replace(/(\.[^.]+)$/, '-cropped$1'), tgt);
      a.click();
      URL.revokeObjectURL(url);
    },
  };
