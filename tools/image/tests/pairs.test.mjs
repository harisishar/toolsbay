...
import { PAIRS, SOURCES, TARGETS } from '../src/content.ts';
...
test('one page per source/target combination, minus the no-ops', () => {
  ...
  for (const p of PAIRS) assert.notEqual(p.src, p.tgt, `${p.slug} converts to itself`);
...
test('slugs follow the src-to-tgt route pattern index.tsx registers', () => {
  for (const p of PAIRS) {
    assert.equal(p.slug, `${p.src}-to-${p.tgt}`, `${p.slug} does not match its src/tgt`);
    assert.match(p.slug, /^[a-z0-9]+-to-[a-z0-9]+$/);
  }
});

test('every target maps to a mime the encoder understands', () => {
  const ENCODABLE = new Set(['image/jpeg', 'image/png', 'image/webp', 'image/x-icon']);
  for (const [key, t] of Object.entries(TARGETS)) {
    assert.ok(ENCODABLE.has(t.mime), `target ${key}: encoder cannot produce ${t.mime}`);
    assert.ok(t.label?.trim() && t.long?.trim() && t.pros?.trim(), `target ${key} incomplete`);
  }
  for (const [key, s] of Object.entries(SOURCES)) {
    assert.ok(s.label?.trim() && s.long?.trim() && s.blurb?.trim(), `source ${key} incomplete`);
  }
});
...
test('pages disclose the information each conversion loses', () => {
  ...
  for (const p of PAIRS) {
    ...
    if (p.src === 'gif') {
      assert.match(faqText(p), /animat/i, `${p.slug} does not mention losing animation`);
    }
  }

  // ICO has a hard 256px ceiling that surprises people converting a photo.
  for (const p of PAIRS.filter((x) => x.tgt === 'ico')) {
    assert.match(faqText(p), /256/, `${p.slug} does not mention the 256px ICO limit`);
    assert.match(faqText(p), /favicon/i, `${p.slug} should explain the favicon use case`);
  }

  assert.equal(byslug['heic-to-jpg'].src, 'heic', 'heic-to-jpg is the tool’s top target');
  assert.match(faqText(byslug['heic-to-jpg']), /iphone/i, 'HEIC pages should name the iPhone');
});
