...
import {
  ...
  eisParts,
  myAnnualTax,
} from "../src/lib/calcs/salary-my.ts";
import {
  usTax,
  ukTax,
  sgTax,
  ...
  caTax,
  jpTax,
} from "../src/lib/calcs/salary-world.ts";
...
// have to stay true as the registry changes.
test("the Calculator.net comparison matches the real registry", () => {
  const slugs = new Set(ALL_CALCS.map((c) => c.slug));
  for (const s of [
    ...
    assert.ok(
      slugs.has(s),
      `the comparison page links /${s}, which does not exist`,
    );
  }

  const salary = ALL_CALCS.filter((c) => c.category === "Salary & Tax");
  for (const c of COMPARISONS) {
    const prose = [c.intro, ...c.sections.flatMap((s) => s.body)].join(" ");
    assert.ok(
      prose.includes(String(ALL_CALCS.length)),
      `${c.slug} quotes a calculator count that is not ${ALL_CALCS.length}`,
    );
    assert.ok(
      prose.includes(String(salary.length)),
      `${c.slug} quotes a Salary & Tax count that is not ${salary.length}`,
    );
    // Conceding their breadth is the credibility of the whole page.
    assert.ok(
      c.matrix.some(
        (r) => /number of calculators/i.test(r.feature) && /~?200/.test(r.them),
      ),
      `${c.slug} must state Calculator.net's larger inventory`,
    );
  }
});
...
test("registry: unique slugs, all compute with defaults", () => {
  ...
  assert.ok(
    ALL_CALCS.length >= 50,
    `expected 50+ calculators, got ${ALL_CALCS.length}`,
  );
  for (const calc of ALL_CALCS) {
    const v = Object.fromEntries(
      calc.inputs.map((i) => [i.key, String(i.def ?? "")]),
    );
    const r = calc.compute(v);
    assert.ok(
      Array.isArray(r.rows) && r.rows.length > 0,
      `${calc.slug} returned no rows`,
    );
    for (const row of r.rows) {
      assert.ok(
        typeof row.value === "string" && !row.value.includes("NaN"),
        `${calc.slug}: ${row.label} -> ${row.value}`,
      );
    }
  }
});
...
test("spot-check a few full compute() paths", () => {
  const loan = CALC_BY_SLUG["loan-calculator"].compute({
    amount: "20000",
    rate: "6.5",
    years: "5",
  });
  assert.match(loan.rows[0].value, /391\.32/);
  const bmi = CALC_BY_SLUG["bmi-calculator"].compute({
    height: "170",
    weight: "70",
  });
  assert.match(bmi.rows[0].value, /24\.2/);
  const subnet = CALC_BY_SLUG["subnet-calculator"].compute({
    cidr: "192.168.1.10/24",
  });
  assert.match(subnet.rows[0].value, /192\.168\.1\.0\/24/);
  assert.match(subnet.rows[4].value, /254/);
  const my = CALC_BY_SLUG["malaysia-salary-calculator"].compute({
    salary: "6000",
    bonusMonths: "0",
  });
