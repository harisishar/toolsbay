...
import { TOOLS, CATEGORIES } from '../src/content.ts';
...
test('every tool has the SEO fields the page template renders', () => {
  for (const t of TOOLS) {
    ...
    assert.ok(CATEGORIES.includes(t.category), `${t.slug}: bad category ${t.category}`);
    // Long titles get truncated in the SERP; long descriptions get rewritten.
    assert.ok(t.title.length <= 75, `${t.slug}: title ${t.title.length} chars (>75)`);
    assert.ok(t.desc.length <= 175, `${t.slug}: desc ${t.desc.length} chars (>175)`);
  }
});
...
test('in-browser tools do say the files stay local', () => {
  for (const t of TOOLS.filter((x) => !x.server)) {
    assert.ok(
      t.faq.some((f) => /never leave your device|entirely in your browser/i.test(f.a)),
...
test('presets are valid for their component', () => {
  ...
  for (const t of TOOLS.filter((x) => x.preset !== undefined)) {
    ...
    assert.match(t.preset, /^[a-z]+$/, `${t.slug}: preset must be a bare identifier`);
  }
});
